...
class InputMoneyRequestTest {
    ...
    void calculateLottoCount메서드는_로또티켓구매_금액을_받아_만들_수_있는_로또티켓개수을_응답한다(Money playerMoney, long ticketCountExpected) {
        long ticketCount = playerMoney.calculateLottoCount(1000);
    ...
    void isDivided메서드는_로또티켓구매_금액을_받아_로또티켓가격으로_나누어_떨어지면_true를_응답한다(Money playerMoney) {

        boolean isDivided = playerMoney.isDivided(1000);
    ...
    void isDivided메서드는_로또티켓구매_금액을_받아_로또티켓가격으로_나누어_떨어지지_않으면_false를_응답한다(Money playerMoney) {
        boolean isDivided = playerMoney.isDivided(1000);
